...
sample_im = dispim(reshape(U,Ny,Nx,Nz,[]));
sample_im = abs(sample_im(:,:,1,1)/max(sample_im(:)));
...
recon = abs(reshape(reshape(dispim(reshape(U,Ny,Nx,Nz,[])),[],L)*Phi_rt_temp(:,(ctemp-1)*segtemp+1:(ctemp)*segtemp),Ny,Nx,[]));
% recon=abs(dispim(reshape(reshape(U,Ny*Nx,[])*Phi_rt_temp(:,(ctemp-1)*segtemp+1:(ctemp)*segtemp),Ny,Nx,[])));
cstep=cstep+1;
    tempmask=repmat(mask_s1,[1,1,size(recon,3)]);
    tempmasksig=recon(tempmask);
    masksig((cstep-1)*segtemp+1:(cstep-1)*segtemp+size(recon,3),:)=reshape(tempmasksig,sum(mask_s1(:)),[])';
