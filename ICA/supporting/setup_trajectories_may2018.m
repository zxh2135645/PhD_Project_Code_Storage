...
thetas=mod(theta*mod((1:size(kspace_data,1))+cutoffline-1,2*trajs),360);%RY correct for the starting angle
...
ksig=zeros(size(cand_coords,2),size(kspace_data,2),size(kspace_data,4));
nsig=ksig;
nocands=[];
for j=1:size(cand_coords,2)
...
if isVE %force all angles into [0, 180)-degree range
  ...
  thetas=mod(theta*mod((1:size(kspace_data,1))+cutoffline-1,trajs),180);%RY correct for the starting angle
