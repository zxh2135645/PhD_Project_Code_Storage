function fitResult = fit_T1rhoT2IRVFA_flow(fitParams)
...
%         fitmat = [];
        ...
        parfor j = 1:size(recontemp,1)
            ...
            elseif (ChooseConst == 1) || (ChooseConst == 2)            
...
%                 tempfit(numel(x0)+2) = sqrt(res)*abs(normcurve);
        ...
        if ChooseConst == 0
            ...
            BT2map = imageOrientLPS(abs(wmedfilt2(fitResult.BT2map(:,:,sl,cphase))),params);
            figure(25),imagesc(BT2map,[0.5 1]),axis equal tight,colormap(parula);colorbar;title('T2prep Inversion Efficiency');
        ...
        % display maps
        T1map = imageOrientLPS(abs(wmedfilt2(fitResult.T1map(:,:,sl,cphase))*1000),params);
        figure(21),imagesc(T1map,[0 maxT1*1000]),axis equal tight,colormap(warmmetal);colorbar;title('T1 (msec)');
        
        if numT2prep > 0
            T2map = imageOrientLPS(abs(wmedfilt2(fitResult.T2map(:,:,sl,cphase))*1000),params);
            figure(22),imagesc(T2map,[0 maxT2*1000]),axis equal tight,colormap(warmmetal);colorbar;title('T2 (msec)');
        end
        
        if numT1rhoPrep > 0
            T1rhomap = imageOrientLPS(abs(wmedfilt2(fitResult.T1rhomap(:,:,sl,cphase))*1000),params);
            figure(26),imagesc(T1rhomap,[0 maxT1rho*1000]),axis equal tight,colormap(warmmetal);colorbar;title('T1rho (msec)');
        end

        B1map = imageOrientLPS(abs(wmedfilt2(fitResult.B1map(:,:,sl,cphase))),params);
        figure(23),imagesc(B1map,[0 1]),axis equal tight,colormap(viridis);colorbar;title('FA Scaling');

        BIRmap = imageOrientLPS(abs(wmedfilt2(fitResult.BIRmap(:,:,sl,cphase))),params);
        figure(24),imagesc(BIRmap,[0.5 1]),axis equal tight,colormap(parula);colorbar;title('Inversion Efficiency');
