...
function [r2starmap,residual] = estimateR2starGivenFieldmap ( imDataParams, algoParams, fm, ff )
...
for kt = 1:N
    for kc = 1:C*num_acqs
      images2(:,:,kt,kc) = squeeze(images(:,:,kt,kc)).*exp(-1j*2*pi*fm*t(kt));
    end
end

r2s = linspace(range_r2star(1),range_r2star(2),NUM_R2STARS);
weighting = [5 5 4 3 2 1];
weighting = weighting(1:N);
...
%     temp = reshape(temp,[N C*num_acqs*sy]);
%     temp2 = reshape(sum(abs(reshape(P*temp,[N NUM_R2STARS*C*num_acqs*sy]).*weighting(:)).^2,1),[NUM_R2STARS C*num_acqs*sy]).';
%     temp2 = sum(reshape(temp2,[C*num_acqs sy*NUM_R2STARS]),1);
...
if nargin > 3
    ...
    for kx = 1:sx
        for ky = 1:sy
            fw = [1-ff(kx,ky);ff(kx,ky)];
            Phi1 = Phi * fw;
            P = []; matI = [];
            for k = 1:NUM_R2STARS
                Psi = diag(exp(-r2s(k)*t));
                P = [P;(Psi*Phi*pinv(Psi*Phi))];
                matI = [matI;eye(N)];
            end
            temp = reshape(squeeze(images2(kx,ky,:,:)),[N C*num_acqs]);
            temp2 = reshape(sum((abs(abs(reshape(matI*temp,[N NUM_R2STARS*C*num_acqs])) - abs(reshape(P*temp,[N NUM_R2STARS*C*num_acqs]))).*weighting(:)).^2,1),[NUM_R2STARS C*num_acqs]).';
            temp2 = sum(reshape(temp2,[C*num_acqs NUM_R2STARS]),1);
            residual(kx,ky,:) = temp2;
        end
    end
else
    P = []; matI = [];
    ...
    for kx = 1:sx
        temp = reshape(squeeze(images2(kx,:,:,:)),[sy N*C*num_acqs]).';
        temp = reshape(temp,[N C*num_acqs*sy]);
        temp2 = reshape(sum((abs(abs(reshape(matI*temp,[N NUM_R2STARS*C*num_acqs*sy])) - abs(reshape(P*temp,[N NUM_R2STARS*C*num_acqs*sy]))).*weighting(:)).^2,1),[NUM_R2STARS C*num_acqs*sy]).';
        temp2 = sum(reshape(temp2,[C*num_acqs sy*NUM_R2STARS]),1);
        residual(kx,:,:) = reshape(temp2,[sy NUM_R2STARS]);
    end
end
...
%figure;imagesc(r2starmap,[range_r2star(1) range_r2star(2)]);colormap(inferno);axis equal off tight;colorbar;
