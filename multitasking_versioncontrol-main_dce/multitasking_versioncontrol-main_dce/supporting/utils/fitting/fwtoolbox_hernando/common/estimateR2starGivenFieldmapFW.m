...
function [r2starmap,residual] = estimateR2starGivenFieldmapFW( imDataParams, algoParams, fm )
...
% figure;
% for k=1:10
%     %plot(realify(diag(temp(:,k))*Phi));hold on;
%     plot(realify(diag(temp(:,k))*Phi*[0.2;0.8]));hold on;
%   %plot(realify((diag(temp(:,k))*Phi)*pinv(diag(temp(:,k))*Phi)*temp(:,k)));hold on;
% end
...
for kx = 1:sx
    for ky = 1:sy
        Phi = getPhiMatrixMultipeak(deltaF,relAmps,t)*[algoParams.wf(kx,ky);algoParams.ff(kx,ky)];
        P = [];
        for k = 1:NUM_R2STARS
        ...
        temp2 = reshape(sum(abs(reshape(P*temp,[N NUM_R2STARS*C*num_acqs])).^2,1),[NUM_R2STARS C*num_acqs]).';
        temp2 = sum(reshape(temp2,[C*num_acqs NUM_R2STARS]),1);
        residual(kx,ky,:) = reshape(temp2,[1 1 NUM_R2STARS]);
%     for ky = 1:sy
%         residual(kx,ky,:) = smooth(residual(kx,ky,:));
%     end
    end
end
