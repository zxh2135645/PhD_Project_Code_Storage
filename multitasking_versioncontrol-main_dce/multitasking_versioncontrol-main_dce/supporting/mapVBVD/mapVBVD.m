...
for s=1:NScans
    ...
    if isempty(mdh_syncdata)
        fprintf('No PMU data in rawdata. Checking iceParam... ');
    else
        try
    ...
    % merge PMU data from the unified multitasking sequence
    try
        containsSignal = false;
        if mean(twix_obj{s}.image.iceParam(14,:)) > 0
            % recorded ECG trigger delay has a temporal resolution of 2.5ms
            twix_obj{s}.PMUdata.TriggerDelay = twix_obj{s}.image.iceParam(14,:)*2.5;
            containsSignal = true;
        end
        if mean(twix_obj{s}.image.iceParam(15,:)) > 0
            twix_obj{s}.PMUdata.EKG = twix_obj{s}.image.iceParam(15:17,:);
            containsSignal = true;
        end
        if mean(twix_obj{s}.image.iceParam(18,:)) > 0
            twix_obj{s}.PMUdata.PULS = twix_obj{s}.image.iceParam(18,:);
            containsSignal = true;
        end
        if mean(twix_obj{s}.image.iceParam(19,:)) > 0
            twix_obj{s}.PMUdata.EXT = twix_obj{s}.image.iceParam(19,:);
            containsSignal = true;
        end
        if mean(twix_obj{s}.image.iceParam(20,:)) > 0
            twix_obj{s}.PMUdata.RESP = twix_obj{s}.image.iceParam(20,:);
            containsSignal = true;
        end
        if containsSignal
            fprintf('done.\n');
        else
            fprintf('no PMU data in iceParam.\n');
        end
    catch errormsg
        fprintf('no PMU data in iceParam.\n');
        fprintf('%s\n', errormsg.message);
    ...
    try
        twix_obj{s}.PMUdata.physio_all = [twix_obj{s}.PMUdata.EKG(1:3,:);twix_obj{s}.PMUdata.PULS;twix_obj{s}.PMUdata.EXT(1,:);twix_obj{s}.PMUdata.RESP;twix_obj{s}.PMUdata.TriggerDelay];
    catch
        if twix_obj{s}.image.NAcq == 0
            twix_obj{s}.PMUdata.physio_all = zeros(7,size(twix_obj{s}.RTfeedback.iceParam,2));
        else
            twix_obj{s}.PMUdata.physio_all = zeros(7,size(twix_obj{s}.image.iceParam,2));
        end
    end
        
    % ---------------------------------------------------------------------
    
    clear  mdh  tmpMdh  filePos  isCurrScan  mdh_pmu mdh_syncdata
...
try
    ...
    mdh.EVNT.data = zeros(size(timeStamp40,1),1);
    mdh.EVNT.TimeStamp = timeStamp40;
end
