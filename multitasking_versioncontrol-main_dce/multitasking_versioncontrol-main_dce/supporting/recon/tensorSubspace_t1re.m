function [dataArray,temporalBasis] = tensorSubspace_t1re(tempStructTensor,params,reconOptions,dataArray,temporalBasis)
...
% Define parameters for T1 recovery basis functions
T1_values = linspace(100, 2000, 100); % T1 values in ms
TR = params.lEchoSpacing*params.lSegments;                              % Repetition time in ms 
flip_angle = 0.9 * pi / 180;          % Flip angle in radians
n = 1:50;                            % Sequence indices
x2 = 1;                              % Scaling parameter (x(2))
x3 = 1;                              % Amplitude scaling factor (x(3))

% Preallocate the basis dictionary
curvePhi = zeros(length(n), length(T1_values));

% Generate the basis functions
for i = 1:length(T1_values)
    x1 = T1_values(i); % T1 value
    for j = 1:length(n)
        curvePhi(j, i) = x3 * (1 - exp(-TR / x1)) / (1 - cos(flip_angle) * exp(-TR / x1)) * ...
            (1 + (x2 - 1) * (exp(-TR / x1) * cos(flip_angle))^(n(j) - 1));
    end
end

% Normalize the dictionary
curvePhi = curvePhi ./ vecnorm(curvePhi);
...
% end
%
Phi=C*(curvePhi*(pinv(curvePhi)*UU'));
%Phi = C*UU';
clear UU;
...
Phi_rt_full  = degate((curvePhi*pinv(curvePhi)*Phi(:,:).').',sizes,Segidx_full,Hidx_full,Ridx_full,wallClock_full,echoIdx_full); %(C*UU').'
