function Eff_result = Eff_cellfunc_flow(T1,T2,T1rho,alphaArray,BIR,BT2,TR,TEs,TSLs,Nseg,invSign)
...
for shot = 2:moduleLength
    ...
    if  TEs(shot-1) == 0 && TSLs(shot-1) == 0   % IR pulse
        Eff_result(shot) = Mss_scale(exp(-TR/T1),FA1,FA2) * (1 + ((exp(-TR/T1)*cos(FA1))^Nseg).'*(BIR*cos(pi)*Eff_result(shot-1)-1)); %B is relative B1 field, not inversion efficiency
    else                                        % T2-IR pulse
        Eff_result(shot) = Mss_scale(exp(-TR/T1),FA1,FA2) * (1 + ((exp(-TR/T1)*cos(FA1))^Nseg).'*(BT2*invSign(shot-1)*sin(pi/2)^2*exp(-TEs(shot-1)/T2)*exp(-TSLs(shot-1)/T1rho)*Eff_result(shot-1)-1)); 
    end
...
for shot = 1:moduleLength
    ...
    if  TEs(shot) == 0 && TSLs(shot) == 0   % IR pulse
        c = Mss_scale(exp(-TR/T1),FA1,FA2) * ((exp(-TR/T1)*cos(FA1))^Nseg).'*BIR*cos(pi);
        d = Mss_scale(exp(-TR/T1),FA1,FA2) * (1 + ((exp(-TR/T1)*cos(FA1))^Nseg).'*(-1));
    else                                    % T2-IR pulse
        c = Mss_scale(exp(-TR/T1),FA1,FA2) * ((exp(-TR/T1)*cos(FA1))^Nseg).'*(BT2*(invSign(shot)*sin(pi/2)^2*exp(-TEs(shot)/T2)*exp(-TSLs(shot)/T1rho))); 
        d = Mss_scale(exp(-TR/T1),FA1,FA2) * (1 + ((exp(-TR/T1)*cos(FA1))^Nseg).'*(-1)); 
    end
