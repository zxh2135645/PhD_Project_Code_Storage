...
if cbins > 1
    ...
    if selectROI
%         % 2D hanning window on selected region
%         % User input: draw circular ROI
...
%         mask = repmat(mask,[1 1 Nz]);
        ...
        % User input: heart ROI 
        h = figure;imagesc(sqrt(abs(fbpComposite(floor(Ny/2)-floor(Nydisp/2) + (1:Nydisp), floor(Nx/2)-floor(Nxdisp/2) + (1:Nxdisp), dispSlice,1))));axis equal tight;colormap('gray');title('Draw a rectangular ROI encloses the whole heart')
        roiResp = imrect;
        roiPosition = roiResp.getPosition();
        roiPosition = round(roiPosition);
        close(h);
...
%         roi_weighting(:,:,Nzshift+1+Nzorig:end) = 0;
        ...
        % real-time image of the first 20 seconds
        ...
        maskb = zeros(size(recon,1),size(recon,2));
        maskb(Nyshiftb+(1:ROINyb),Nxshiftb+(1:ROINxb)) = hanning(ROINyb)*hanning(ROINxb).'; 
        maskb(maskb>0.05)  = 1;
        maskb(maskb<=0.05) = 0;
        
        mask = zeros(Ny,Nx);
        mask(Nyshift+(1:ROINy),Nxshift+(1:ROINx)) = maskb;
        mask = repmat(mask,[1 1 Nz]);
        
        % average blood signal curve
        mask1  = maskb(:);
        recon1 = reshape(recon,numel(maskb),[]);
        recon1 = realify(recon1,'rows');
        curve_blood = (mean(recon1(mask1>0,:))).';
        curve_blood(:,2) = 1;
        
        % linear regression to blood signal curve
        ...
        % rough LV/heart mask using LV blood ROI as seed for region growing
        maskh = grayconnected(mean(abs(recon.*windowfun),3),floor(Nyshiftb+ROINyb/2),floor(Nxshiftb+ROINxb/2));
        
        % generate blood mask from regions of low residual
        temp1 = reshape(resnorm,size(maskb,1),size(maskb,2));
        temp1(temp1>mean(temp1(:)))  = 0;
        temp1(temp1>mean(temp1(maskh>0))) = 0;
        %figure;imagesc(temp1);

        % generate heart mask by growing the blood mask
        se = strel('disk',3);
...
%         figure;imagesc(mask2);
        ...
        % ROI weighting for U and Phi_rt_small
        roi_weighting = zeros(Ny,Nx,Nz);
        roi_weighting(Nyshift+(1:ROINy),Nxshift+(1:ROINx),:) = repmat(mask2,[1 1 Nz]);      
    else
...
%         ycenter = sum(weightedy(:)) / sum(imgs_h(:));
        ...
        % Define window
        Ncardy = 2*floor(3*Ny/8);
        Ncardx = 2*floor(3*Nx/8);
        Nyshift = floor(Ny/2) - Ncardy/2;
        Nxshift = floor(Nx/2) - Ncardx/2;
        Nzshift = floor(Nz/2)- floor(Nzorig/2);
        windowfun = zeros(Ny,Nx);
        windowfun(Nyshift+(1:Ncardy),Nxshift+(1:Ncardx)) = hanning(Ncardy)*hanning(Ncardx).';   % ROI is center region of center slices
        roi_weighting = zeros(Ny,Nx,Nz);
        roi_weighting(:,:,Nzshift+(1:Nzorig)) = repmat(windowfun,[1 1 Nzorig]);
    ...
    winwidth = 0.035/dt; %35 ms window width
...
%     end
    ...
    if sum(abs(diff(diff(navTiming)))) >= 1e-6
        ...
        bestres = interp1(newTiming,bestres,navTiming,'nearest','extrap');
...
dataArray.Hidx = row(Hidx);
dataArray.bestresCard = bestresCard;
dataArray.bestres     = row(bestres);
dataArray.wallClockCardiac = wall_clock;
...
mask = dispim(mask);
dataArray.diastoleIdx = 1; %testLVvolume(squeeze(abs(temp(:,:,1,:,1))),mask);
...
%             end
%         else
         Z = realify(Phi_rt_motion/norm(Phi_rt_motion));   
