function recovered = LRTCp_splines(undersampled,lambda,mus,mask,iterations,init_guess,curveU,orders,circs,ts_proj,flagCommandLine)
...
if nargin > 6
    ...
    if fixed_flag
        [curveU,~,~]=svd(double(curveU),'econ'); %orthonormalize
    end
else
    fixed_flag = false;
end
...
% profile on
fprintf('\nLRTCp_splines: %d iterations',iterations);
for i = 1:(iterations+1)
    ...
    for dim = 1:numel(smdims)
        ...
        if function_method
            dr2{dim} = rho2{dim}*norm(vec(reshape(double(ts_proj).'*reshape(Z2{dim}-Z2new{dim},sizes_big(1),[]),[],size(G{dim},2))*G{dim}.'));
        else
  ...
  %   end
    ...
    fprintf(2, '\n%s\n', errormsg.message);
    fprintf('tensor subspace calculation: LRTCp_splines failed. \n');
end
