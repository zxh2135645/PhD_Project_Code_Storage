function [curveU, curveS, fitParams] = genCurveSubspace(params, fitParams, reconOptions)
...
invSign = -(T2s.*(isT2IR-0.5) + T1rhos.*(isT1rhoIR-0.5));
invSign = sign(invSign);
...
Sint = @(A,e1,e2,e1rho,BalphaArray,BIR,BT2,Eff) A .* Mss(e1,BalphaArray) .* (1 + (step(e1,BalphaArray)) .* (((cos(BIR*pi)*IRs + (invSign.*sin(BT2*pi/2)^2).*T2s + (invSign.*sin(BT2*pi/2)^2).*T1rhos).*e2.*e1rho + cos(BT2*pi/2)^2.*(T2s+T1rhos)).*Eff-1)) .* sin_step(BalphaArray);
