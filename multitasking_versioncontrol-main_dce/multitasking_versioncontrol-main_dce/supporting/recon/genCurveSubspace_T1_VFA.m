function [curveU, curveS, fitParams] = genCurveSubspace_T1_VFA(params, fitParams, reconOptions, useBT2)
...
Sint = @(A,e1,e2,e1rho,BalphaArray,BIR,BT2,Eff) A .* Mss(e1,BalphaArray) .* (1 + (step(e1,BalphaArray)) .* ((repmat(BIR*cos(pi)*ones(1,numIRns),1,rep_VE).*e2.*e1rho).*Eff-1)) .* sin_step(BalphaArray);

% if Nz > 1 && MBfactor == 1
    S = @(A,T1,T2,T1rho,Beta,BIR,BT2) row(Sint(  A, e1(T1), e2(T2), e1rho(T1rho), Beta*alphaArray, BIR, BT2, Eff_cellfunc_flow(T1,T2,T1rho,Beta*alphaArray,BIR,BT2,TR,TEs,TSLs,Nseg)));
% else
%     S = @(A,T1,T2,T1rho,Beta,BIR,BT2) row(Sint(  A, e1(T1), e2(T2), e1rho(T1rho), Beta*alphaArray*.9387, BIR, BT2, Eff_cellfunc(T1,T2,T1rho,Beta*alphaArray*.9387,BIR,BT2,TR,TEs,TSLs,Nseg))...
...
%                                         + Sint(2*A, e1(T1), e2(T2), e1rho(T1rho), Beta*alphaArray*.0525, BIR, BT2, Eff_cellfunc(T1,T2,T1rho,Beta*alphaArray*.0525,BIR,BT2,TR,TEs,TSLs,Nseg)));
% end
...
Betas = linspace(0.1, 1.0, 9);
if strcmp(ScanType, 'SR')
    BIRs = linspace(0.3, 0.6, 5);
else
    BIRs = linspace(0.6, 2, 11);
end
Bflows = 1;
            ...
            for m = 1:numel(Betas)
                for n = 1:numel(BIRs)
                    for p = 1:numel(Bflows)
                        curves(:,j,k,l,m,n,p) = S(T1w(j),T1s(j),T2s(k),T1rhos(l),Betas(m),BIRs(n),1);
