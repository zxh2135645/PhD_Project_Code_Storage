function [s_cbd, im_c_cbd, ims, s_noreg] = sensemaps_cbd_old(ims,kernel_size,varargin)
...
% Create coil combination image: sum-of-squares magnitude & first eigenimage phase
...
%     hmask(1:kernel_size(1),1:kernel_size(2)) = hanning(kernel_size(1))*hanning(kernel_size(2))';
...
if acs_flag
    ...
    [temp,flag] = pcg(AhA,Ah(kspace),[],1,[],[],im_c(:));
...
try
    for it = 1:its
        if acs_flag
             adjh    = @(x)vec(fftim(bsxfun(@times,conj(im_c_cbd),ifftim(kspace_mask.*prep(x,sizes)))).*hmask);
...
%         if acs_flag
%             adjh    = @(x)vec(bsxfun(@times,conj(im_c_cbd),ifftim(kspace_mask.*prep(x,sizes).*hmask)));
%             adjfwdh = @(h)vec(bsxfun(@times,conj(im_c_cbd),ifftim(kspace_mask.^2.*fftim(bsxfun(@times,im_c_cbd,prep(s_cbd,sizes))).*hmask)));
%             adjhb   = adjh(kspace);
%         else
%             adjh    = @(x)vec(fftim(bsxfun(@times,conj(im_c_cbd),prep(x,sizes))).*hmask);
%             adjfwdh = @(h)vec(fftim(bsxfun(@times,abs(im_c_cbd).^2,ifftim(prep(h,sizes).*hmask))).*hmask);
...
%         end
%         
%         [temp,flag] = pcg(@(x)adjfwdh(x),adjhb,[],10,[],[],s_cbd(:));
%         s_cbd = reshape(temp,Np,Nf,Ns,coils);
%         s_cbd = ifftim(fftim(s_cbd).*hmask);
    ...
    fprintf('Sensitivity map estimation failed.\n');
end
delete(progress);
