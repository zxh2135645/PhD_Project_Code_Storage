...
addpath(genpath('../FFMap_IDEAL_fromChris'));
rmpath(genpath('../FFMap_IDEAL_fromChris/Example/'));
...
% dicom_folder = '3D_mGRE_Carlos'; % Carlos
% dicom_folder = '3D_mGRE'; % George
% dicom_folder = '3D_mGRE_Roger'; % Roger
dicom_folder = '3D_mGRE_Tony'; % Tony
...
% Carlos
% [img_mag slice_data_mag] = dicom23D(cat(2, dicom_dir, '/', dicom_folder, '_MAG_31001/'), dicom_fields);
% [img_phase slice_data_phase] = dicom23D(cat(2, dicom_dir, '/', dicom_folder, '_PHASE_32001/'), dicom_fields);

% [img_mag slice_data_mag] = dicom23D(cat(2, dicom_dir, '/', dicom_folder, '_MAG_23001/'), dicom_fields);
% [img_phase slice_data_phase] = dicom23D(cat(2, dicom_dir, '/', dicom_folder, '_PHASE_24001/'), dicom_fields);

% [img_mag slice_data_mag] = dicom23D(cat(2, dicom_dir, '/', dicom_folder, '_MAG_51001/'), dicom_fields);
% [img_phase slice_data_phase] = dicom23D(cat(2, dicom_dir, '/', dicom_folder, '_PHASE_52001/'), dicom_fields);

[img_mag slice_data_mag] = dicom23D(cat(2, dicom_dir, '/', dicom_folder, '_MAG_71001/'), dicom_fields);
[img_phase slice_data_phase] = dicom23D(cat(2, dicom_dir, '/', dicom_folder, '_PHASE_72001/'), dicom_fields);
...
%%
iField_trunc = iField(:,:,8,:); % Carlos, George
...
%%
mask_f = GetFullPath(cat(2, dicom_dir, '/Results/mask_roi.mat'));
...
figure;
subplot(2,2,1);
imagesc(abs(fat(:,:)));colorbar;colormap jet; caxis([0,0.2]);
subplot(2,2,2);
...
subplot(2,2,4);
imagesc(abs(fitting_error(:,:)));colorbar;colormap jet;
...
figure(); 
imagesc(ff(:,:));caxis([0 0.2]);
colormap(brewermap([],'*RdYlBu')); colorbar;

figure(); 
imagesc(fitting_error(:,:));caxis([0 0.2]);

figure(); 
imagesc(R2s(:,:));caxis([0 100]);
...
if FlagSaveB0
    resdir=GetFullPath([dicom_dir,'/Results/']);
    if ~exist(resdir)
        mkdir(resdir);
    end
