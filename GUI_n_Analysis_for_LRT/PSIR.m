...
for ii = 1:1
    ...
    for i = 1:1
        ...
        for te = 1:NEco_old
            ...
            phase_img = angle(temp_sec(:,:,:));
            phase_img_cos = cos(phase_img(:,:,:));
            phase_img_cos(phase_img_cos >= 0) = 1;
            phase_img_cos(phase_img_cos < 0) = -1;
            

            mag_psir = abs(temp_sec(:,:,:)).* phase_img_cos(:,:,:);
            mag_psir = (mag_psir - min(mag_psir(:))) ./ (max(mag_psir(:)) - min(mag_psir(:)));


            mag_psir_v2 = abs(temp_sec(:,:,num_seg_array)).* cos_phase_diff(:,:,:);
            mag_psir_v2 = (mag_psir_v2 - min(mag_psir_v2(:))) ./ (max(mag_psir_v2(:)) - min(mag_psir_v2(:)));

            % Phase unwrapping algorithm??

            phase_img = (phase_img - min(phase_img(:))) ./ (max(phase_img(:)) - min(phase_img(:)));
            figure();
            subplot(1,2,1);
