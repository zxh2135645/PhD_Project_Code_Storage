...
width_max = [0.6, 2.0];
width_max = [0.4, 0.8, 1.0, 3.0, 4.0];
width_max = [0.4, 0.6, 0.8, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0];
...
% for cc = 1:1
    ...
    % for w = 1:1
        ...
        C_cell = cell(length(res_array), length(sigma_array));
        ...
        % for s = 1:1
            ...
            %for i = 6:6
                res = res_array(i);
                Nres = round(res / dx);
                C = zeros([Ny, Nx, Nres, size(t,3)]);
                for te = 1:length(TE_array)
                    C(:,:,:,te) = Func_map_to_bloc_Exaustive(dx, Nx, res, t(:,:,te));
                end
                C_gre = reshape(C, [], length(TE_array));
                C_t2star_fit = zeros(Nx*Ny, 1);
                % C_t2star_fit_reshape = zeros(size(C_gre, 1), 1);
                %options = fitoptions('Method', 'NonlinearLeastSquares');
                %options.Lower = [0 -10];
                %options.Upper = [1, -0.01];

                % Reshape C_gre for parfor
                % Need a different strategy to reshape
                % C_gre_temp = zeros(Nx*Ny, size(C_gre,2));
                achr = 0;
                N = 0;
                C_gre_temp = [];
                C_gre_len = [];
                sld_prev = 1;
                for sld = 1:size(C_gre,1)
                    if achr ~= C_gre(sld, 1)
                        sld_next = sld;
                        N = N + 1;
                        C_gre_temp(N,:) = C_gre(sld,:);
                        if N ~= 1
                            C_gre_len(N-1) = sld_next - sld_prev;
                        end
                        sld_prev = sld;
                    end
                    achr = C_gre(sld,1);
                    %C_gre_temp(sld,:) = C_gre((sld-1)*Nres+1,:);
                end
                C_gre_len(N) = sld - sld_prev + 1;
                %for sld = 1:size(C_gre_temp,1)
                %    C_gre_temp(sld,:) = C_gre((sld-1)*Nres+1,:);
                %end
                 
                parfor ii = 1:size(C_gre_temp,1) %*length(res_array)
                    f_t = fit(TE_array, C_gre_temp(ii,:)', 'exp1', 'Lower', [0 -10], 'Upper', [1 -0.01]);
                    C_t2star_fit(ii) = -1/f_t.b;
                    %f_remote = fit(TE_array, remote_gre, 'exp1');
                end

                C_t2star_fit_reshape = [];
                for sld = 1:size(C_t2star_fit,1)
                    C_t2star_fit_reshape = [C_t2star_fit_reshape, repelem(C_t2star_fit(sld), C_gre_len(sld))];
                end
                
                % C_t2star_fit_reshape = repelem(C_t2star_fit, Nres);
                % C_t2star_fit_reshape(:,:,:,s) = reshape(C_t2star_fit, Ny, Nx, Nres);
                C_cell{i,s} = reshape(C_t2star_fit_reshape, Ny, Nx, Nres);
            end
            %
        end
        toc;
        % tissue_canvas(w).C_t2star_fit_reshape = C_t2star_fit_reshape;
        tissue_canvas(w).C_cell = C_cell;
    end
    tissue_canvas_cell{cc} = tissue_canvas;
end


SimPhantom_04012023.res_array = res_array;
SimPhantom_04012023.sigma_array = sigma_array;
SimPhantom_04012023.width_max = width_max;
SimPhantom_04012023.tissue_canvas = tissue_canvas_cell;

fname = 'SimPhantom_04012023';
save(cat(2, save_dir, '/', fname), '-v7.3', 'SimPhantom_04012023');
