classdef GUI2021 < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function ImageModalityDropDownValueChanged(app, event)
            app.image_modal = app.ImageModalityDropDown.Value;
        end
        ...
        function NameListDropDownValueChanged(app, event)
            app.name = app.NameListDropDown.Value;
        end
        ...
        function GetRealPWD(app, event)
            if isdeployed && ismac
                NameOfDeployedApp = 'GUI2021'; % do not include the '.app' extension
        ...
        function LoadFileButtonPushed(app, event)
            ...
            GetRealPWD(app);
            app.base_dir = cat(2,  app.real_pwd, '/Dicom_ForKhalid/');
            
            % dicom_to_read = cat(2, app.base_dir, 'Dicom/', app.name, '/', app.image_modal, '/');
            f_to_read = cat(2, app.base_dir, 'Data/VE_RES_DATA.mat');
            %app.dicom_glob = glob(cat(2, dicom_to_read, '*.dcm'));
            %GetSliceInfo(app); % Not further used yet
            D = load(f_to_read, app.image_modal);
            Dx = D.(app.image_modal);
...
%             end
            ...
            app.img_3D = permute(Dx{app.subject_idx}, [2,3,1]);
            
            app.slice_idx = 1;
            app.skip_slice_idx = [];
            app.SliceskippedEditField.Value = mat2str(app.skip_slice_idx);
            app.rgb_scale = 0.8;
            app.img_size = size(app.img_3D);
            ...
            app.ax1_rgb(:,:,3) = app.rgb_scale*app.img_3D_rgb + 0.5*edg;
        ...
        function SaveFileButtonPushed(app, event)
            ...
            if all(any(mi_mask_check)) || isSubset % check if all are edited, or if not-edited are skipped
                ...
                save(cat(2, app.save_dir, app.image_modal, '_vol_img_3D.mat'), 'vi3');
                % Status Change               
        ...
        function LoadCorrectedMaskButtonPushed(app, event)
            contour_dir = cat(2, app.base_dir, 'ContourData/');
            app.name_dir = cat(2, contour_dir, app.name, '/');
            load(cat(2, app.name_dir, 'MyoReservoir/mask_myocardium.mat'));
            app.myo_mask = mask_myocardium_3D;

            edg  = edge(squeeze(app.myo_mask(:,:,app.slice_idx)),'Canny');
            app.ax1_rgb(:,:,3) = app.rgb_scale*app.img_for_window + 0.5*edg;
        ...
        function mkdir_in_save_dir(app)
            ...
            app.mi_dir = cat(2, app.save_dir, app.image_modal, '_MI/');
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.ImageModalityDropDownLabel.Text = 'Image Modality';
            
            % Create ImageModalityDropDown
            app.ImageModalityDropDown = uidropdown(app.LeftPanel);
            dropdown_list = ['Please Select One Modality', app.sequence_label];
            ...
            app.NameListDropDownLabel.Text = 'Name List';
            
            % Create NameListDropDown
            app.NameListDropDown = uidropdown(app.LeftPanel);
            
            app.name_glob = glob(cat(2, 'Dicom_ForKhalid/', 'Dicom', '/*/'));
            Names = ExtractNames(app.name_glob);
            app.NameListDropDown.Items = ['Please Select', Names];
