...
if strcmp(domain,'kspace')
    ...
    %%%% normal D2 kernel(original)
    D = 1/3 - (X*B0_dir(1) + Y*B0_dir(2) + Z*B0_dir(3)).^2./(X.^2+Y.^2+Z.^2);
    %%%% Laplacian DL kernel
%     D = 1/3* (X.^2 + Y.^2 + Z.^2) -  (X*B0_dir(1) + Y*B0_dir(2) + Z*B0_dir(3)).^2;
    D(isnan(D)) = 0;
    D = fftshift(D);
...
%     d = (voxel_size(1).*voxel_size(2).*voxel_size(3).*3*( X*B0_dir(1) + Y*B0_dir(2) + Z*B0_dir(3)).^2 - X.^2-Y.^2-Z.^2)./(4*pi*(X.^2+Y.^2+Z.^2).^2.5);
    d(isnan(d)) = 0;
    D = fftn(fftshift(d));
end
