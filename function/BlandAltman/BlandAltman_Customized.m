...
function [rpc, fig, stats] = BlandAltman_Customized(varargin)
...
params.symbols = 'osdp^v';
params.markerSize = 36;
...
params.Legend = 'On';
params.LineWidth = 2; % XZ 05/13/2023
params.color_cell1 = {[254,240,217]/255, [253,204,138]/255, [252,141,89]/255, [227,74,51]/255, [179,0,0]/255};
params.color_cell2 = {[241, 238, 246]/255, [189, 201, 225]/255, [116, 169, 207]/255, [43, 140, 190]/255, [4, 90, 141]/255};
params.cc = [[252,141,89]/255;...
     [189, 201, 225]/255];
% params.cc = [253,204,138]/255;
...
while length(varargin)>i
	...
	switch upper(parameter)
        ...
        case 'MARKERFACECOLORS', params.cc = val;
		otherwise
...
for groupi=1:params.numGroups
	if strcmpi(params.symbols,'Num')
		...
		if params.numGroupsByColor==1
            ...
            markercolor = params.cc(groupi,:); % XZ 05/13/2023
		else
			marker = params.symbols(rem(groupi-1,params.numGroupsBySymbol)+1);
			color = params.colors(floor((groupi-1)/params.numGroupsBySymbol)+1,:);
            markercolor = params.cc(floor((groupi-1)/params.numGroupsBySymbol)+1,:); % XZ 05/13/2023
		end
		ph=scatter(cAH, data.set1((groupi-1)*params.numElementsPerGroup+(1:params.numElementsPerGroup)), data.set2((groupi-1)*params.numElementsPerGroup+(1:params.numElementsPerGroup)),...
			params.markerSize,...
            marker,...
			'MarkerFaceColor',markercolor, ...
            'MarkerEdgeColor',color);
        set(ph,'LineWidth', params.LineWidth); % XZ 05/13/2023
        set(ph,'MarkerFaceAlpha', 0.5);
	end
end
...
plot(cAH, x, y,	'-k', 'LineWidth', params.LineWidth); % XZ 05/13/2023
...
% h = plot(cAH, l, l, ':'); set(h,'color',[0.6 0.6 0.6],'tag','IdentityLine','LineWidth', params.LineWidth); % XZ 05/13/2023
%set(h,'color',[0.6 0.6 0.6],'tag','IdentityLine','LineWidth', params.LineWidth); % XZ 09/14/2023
...
if 0 % Add 95% CI lines
	...
	set(h,'color',[0.6 0.6 0.6],'linestyle','-', 'LineWidth', params.LineWidth); % XZ 05/13/2023
end
corrtext = {};
...
for groupi=1:params.numGroups
	...
	if strcmpi(params.symbols,'Num')
		...
		ph = scatter(baAH,ref,dif,params.markerSize,...
            marker,...
			'MarkerFaceColor',markercolor, ...
            'MarkerEdgeColor',color);
        set(ph, 'LineWidth', params.LineWidth); % XZ 05/13/2023
        set(ph, 'MarkerFaceAlpha', 0.5);        % XZ 05/14/2023
	end
end
axis(baAH,'square')
...
switch upper(params.baStatsMode)
	case {'NORMAL','GAUSSIAN'}
		plot(baAH,a(1:2),stats.differenceMean+[0 0],'k', 'LineWidth', params.LineWidth);  % XZ 05/13/2023
		plot(baAH,a(1:2),stats.differenceMean+stats.rpc*[1 1],':k', 'LineWidth', params.LineWidth); % XZ 05/13/2023
		plot(baAH,a(1:2),stats.differenceMean-stats.rpc*[1 1],':k', 'LineWidth', params.LineWidth); % XZ 05/13/2023
