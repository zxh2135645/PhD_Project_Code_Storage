...
% 11/01/2025 XZ
% Implement sacardialRefPoint and sacardialInferiorRefPoint
...
if any(contour_idx(:))
    ...
    refPoint = cell(1, num_slice);
    inferPoint = cell(1, num_slice);
    ...
    for i = 1:num_slice
        ...
        for j = 1:length(ctype)
            ...
            elseif contains(contour_type, 'sacardialRefPoint')
                % for c = 1: length(con.contours(contour_idx(i)).pts{j})
                refPoint{i} = round(con.contours(contour_idx(i)).pts{j});
                % end
            elseif contains(contour_type, 'sacardialInferiorRefPoint')
                % for c = 1: length(con.contours(contour_idx(i)).pts{j})
                inferPoint{i} = round(con.contours(contour_idx(i)).pts{j});
                % end
            end
        end
    end
