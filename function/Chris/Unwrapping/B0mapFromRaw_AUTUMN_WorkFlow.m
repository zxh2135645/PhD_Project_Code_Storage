% Modified Chris Huang modify for new fit_ppm and suprs_gc function
...
for n=1:(length(ls)-2)  % loop used for field map batch process
    ...
    % make sure data is in 3TE and this part may needed further adjustment, currently is coded for test convenience, CH, 2019.9
    iField_TE3 = zeros(NumRO, NumPE, NumSlices, NumEchos-3, NumCh);
    TE3 = zeros(size(TE,1),size(TE,2)-1);
    iField_TE3(:,:,:,1:2,:) = iField(:,:,:,1:2,:);
    iField_TE3(:,:,:,3,:) = iField(:,:,:,3,:); 
    TE3(1:2) = TE(1:2);
    TE3(3) = TE(3);
...
% combine multiple coils together, assuming the coil is the fifth dimension
    ...
    L_fat = 1; % assume there is fat in test object, L_fat = 0 if there is not
    %[unwph_uf,unwph,N_std,iFreq_raw,Mask] = ...
    %spurs_gc_UNIC_RY_Chris_0916(L_fat,iField(:,:,:,:,:),TE,f_central,voxel_size);
    [unwph_uf,unwph,N_std,iFreq_raw1,iFreq_ref,Mask,CenterF_ref] = ...
        spurs_gc_UNIC_RY_Chris_WorkFlow(L_fat,iField(:,:,:,:,:),TE,f_central,voxel_size); %unwph phase difference
    %[unwph_uf unwph N_std] = spurs_gc_UNIC(iField,AllPhasemap(n).TE/1e6,AllPhasemap(n).Freq,voxel_size);
    [unwph_uf_CenF] = CenterF_Correction(unwph_uf,CenterF_ref,Mask);
    inputphase=unwph_uf_CenF;% unwrap and fat corrected phase map, its the map of phase difference between echo1 and echo2, not field map nor phase image at certain echo
