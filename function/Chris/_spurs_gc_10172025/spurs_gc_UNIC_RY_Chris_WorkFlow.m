% Modidied Chris Huang 2019 July modify for dynamic range 
...
function [wunwph_uf ,unwphw ,N_std ,iFreq_raw,iFreq_ref,Mask1,CenterF_ref] = spurs_gc_UNIC_RY_Chris_WorkFlow(L_Fat,iField,TE,CF,voxel_size,SUBSAMPLE,dfat)
if nargin<6
    SUBSAMPLE = 1;
end
...
if 0 %abs((TE(2)-TE(1))-(TE(3)-TE(2)))< 0.0002
    ...
    [iFreq_raw,N_std,iFreq_ref,CenterF_ref] = Fit_ppm_complex_WorkFlow(iField,TE,Mask);
end
...
if (w1 > 0)
    w = w1;
    %iFreq_raw1 = medfilt3(iFreq_raw1,[3 3 1]);
    [unwphw,iter,erglist] = phase_unwrap_3d_UNIC_Chris_1002(iFreq_raw1,p,iMag,voxel_size,Mask); 
    for i = 1:z
        unwphw(:,:,i) = filloutliers(unwphw(:,:,i),'spline','movmedian',3,2);
        unwphw(:,:,i) = filloutliers(unwphw(:,:,i),'spline','movmedian',3,1);
    end
    energy = erglist;
    if L_Fat == 0
        wunwph_uf = unwphw;
    else
        [wkappa,wm_fat,wunwph_uf,iter,erglist,wkiter] = unwrap_unfat_3dP_UNIC_Chris_1002(voxel_size,iMag,w,unwphw,p,Mask); % a small mistake found here. 2014.2.21
        for i = 1:z
            wunwph_uf(:,:,i) = filloutliers(wunwph_uf(:,:,i),'spline','movmedian',3,2);
            wunwph_uf(:,:,i) = filloutliers(wunwph_uf(:,:,i),'spline','movmedian',3,1);
        end
    end
