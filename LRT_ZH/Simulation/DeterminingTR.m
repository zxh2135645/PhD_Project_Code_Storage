    ...
    for n = 1:length(N_array)
        ...
        for tff = 1:length(flip_prep_array)
            ...
            for t11 = 1:length(T1_array2)
                ...
                Mz_recov_TE_dict_noise{t11, n, tff, its} = Mz_recov_TE_dict{idx, n, tff} + 0.05*randn(1,length(Mz_recov_TE_dict{idx, n, tff}));
            end
        end
    end
end
...
figure();
xtlbl = [160:40:1000];
xt = 1:2:length(xtlbl)*2;
seg_array = [160:20:1000];
...
for t111 = 1:length(T1_array2)
    ...
    y = mean(error_mat(t111,end-26:end,1,2));
    y_sd = std(error_mat(t111,end-26:end,1,2));
    yline(y);
