...
parfor j=1:numel(rows)
    ...
    tempfit=lsqnonlin(@(x)abs(S(Avp(x(1),x(2),x(3)),x(1),x(2),x(3))-curve),...
        [2/3,alpha,median([minB, real(curve(1)/curve(end)), maxB])],...
...
figure,imagesc(fits(:,:,3)*180/pi),colormap(jet(256))
...
figure,imagesc(1./fits(:,:,2),[0 2.3]),colormap(jet(256))
figure,imagesc(fits(:,:,3)*180/pi),colormap(jet(256))
figure,imagesc(fits(:,:,4)),colormap(jet(256))
figure,imagesc(-fits(:,:,4)./fits(:,:,2),[0 2.3]),colormap(jet(256))