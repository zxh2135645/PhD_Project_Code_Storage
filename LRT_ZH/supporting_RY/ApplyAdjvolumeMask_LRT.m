...
% 08/16/2019 Mona,Tim
function Mask_matrix = ApplyAdjvolumeMask_LRT(twix)
...
%% generate coordinate of the LRT recon image
x_LRT=x;
y_LRT=x;
z_LRT=z;

voxel_LRT = [info.img.voxel_size(1) info.img.voxel_size(1) info.img.voxel_size(3)];
voxel_LRT = repmat(voxel_LRT,3,1);
%rotation_img = rotation_matrix(info.img.dSag,info.img.dCor,info.img.dTra,info.img.dRot);
%center_img = [info.img.dSag_center info.img.dCor_center info.img.dTra_center];
temp_affine = [[rotation_img.*voxel_LRT center_img'];[0 0 0 1]];
% corner = temp_affine*[-info.img.arraysize(1)/2 info.img.arraysize(2)/2 -info.img.arraysize(3)/2 1]';
corner_LRT = temp_affine*[-x_LRT/2 y_LRT/2 z_LRT/2 1]';

% get the img affine matrix
corner_recon = [corner_LRT(1) corner_LRT(2) corner_LRT(3)];
img_affine_recon = [[rotation_img.*voxel_LRT corner_recon'];[0 0 0 1]];


tic
word_cordf = zeros([4 x_LRT*y_LRT*z_LRT]);
a = 1;
for i = 1:z_LRT
    for j = 1:y_LRT
...
word_cordf = img_affine_recon*word_cordf;
...
Mask_matrix = reshape(result,[x_LRT,y_LRT,z_LRT]);
toc
%use only the center FOV
Mask_matrix=Mask_matrix(end/4+1:3*end/4,end/4+1:3*end/4,:);
...
% toc
%% phase encoding direction flip
if twix.image.NCol==(twix.hdr.Dicom.lBaseResolution*2) % phase encoding direction at column%% hacked becausre no phase encoding directition dicom tag found
    Mask_matrix=permute(Mask_matrix(:,:,:,:,:), [2 1 3 4 5]);
else % phase encoding direction at row
    %do nothing
end
