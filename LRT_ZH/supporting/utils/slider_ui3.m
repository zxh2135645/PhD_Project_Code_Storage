% Sliders for 2 spatial dimensions and 3 time dimensions. Fast(ish), but high memory usage.
...
javacomponent(jSlider3,[width/2-50,0,100,20]);
...
hjSlider1.StateChangedCallback = @(hjSlider,eventData)...
  imshow(UV(:,:,indround(hjSlider1.getValue,2),indround(jSlider2.getValue,3),indround(jSlider3.getValue,4)));

hjSlider2 = handle(jSlider2, 'CallbackProperties');
hjSlider2.StateChangedCallback = @(hjSlider,eventData)...
  imshow(UV(:,:,indround(jSlider1.getValue,2),indround(hjSlider2.getValue,3),indround(jSlider3.getValue,4)));

hjSlider3 = handle(jSlider3, 'CallbackProperties');
hjSlider3.StateChangedCallback = @(hjSlider,eventData)...
  imshow(UV(:,:,indround(jSlider1.getValue,2),indround(jSlider2.getValue,3),indround(hjSlider3.getValue,4)));