function [MxyTE, BSIM, plotTEidx, SPGRcat, Msimf, SPGRend] = SPGR_engine(M0, T1, T2, TR, TE, FA, NTR, dt, f_vec, PLOT_EACHSPIN, PLOT_SS, varargin)
...
BSIM.RFCYC = 0; % 0:none, 1:SSFP phase cycling, 2:SPGR spoiling
BSIM.RFphi0= 0;
% BSIM.RFdph = 2*0.5*pi;
BSIM.RFdph = 0/180*pi;
...
SPGRcat.catmode  = 0;
SPGRcat.NcatTR   = 0; % for T1 MOLLI, 5 linear ramp-up
SPGRcat.NstepTR  = int32(BSIM.TR/BSIM.dt);
if( SPGRcat.catmode==0 )
    SPGRcat.NstepTot = 0;
elseif( SPGRcat.catmode==1 )
    SPGRcat.NstepTot = int32(SPGRcat.NcatTR*SPGRcat.NstepTR/2);
elseif( SPGRcat.catmode==2 )
    SPGRcat.NstepTot = int32(SPGRcat.NcatTR*SPGRcat.NstepTR);
elseif( SPGRcat.catmode==3 )
    SPGRcat.NstepTot = int32(SPGRcat.NcatTR*SPGRcat.NstepTR);    
else
    fprintf(1,'SPGRcat.catmode = %d not supported\n',SPGRcat.catmode);
    return;
end   

% Ramp-down at the end - this is not applicable for SPGR
if RAMP_DOWN == 0
    SPGRend.RAMP_DOWN = 0;
    SPGRend.NendTR = 0; 
elseif RAMP_DOWN == 1
    SPGRend.RAMP_DOWN = 1;
    SPGRend.NendTR = 1; % Not sure if ramp-down is TR or TR/2
end
SPGRend.NstepTR = int32(BSIM.TR/BSIM.dt); % Not sure if ramp-down is TR or TR/2
SPGRend.NstepTot = int32(SPGRend.NendTR*SPGRend.NstepTR);
...
Msimf = zeros(3, BSIM.NstepTot + SPGRcat.NstepTot + SPGRend.NstepTot, Nf);
...
for f=1:Nf
    ...
    if( SPGRcat.catmode>0 )
        ...
        % TODO
        % Gradient Phase spoiling for SPGR
    ...
    % Sim TR by TR
    % TODO -> add gradient spoiling
    %spoil_phi_vec = linspace(0,4*pi,length(f_vec));
    %spoil_phi = spoil_phi_vec(f);
    spoil_phi = 0;

    [Msim, RF_phi] = SRsimfunc(BSIM, spoil_phi); 
    % update magnetization history
    BSIM.M = Msim(:,end);
    ...
    % Thus, for SPGR, RAMP_DOWN = 0;
    if (SPGRend.RAMP_DOWN == 1)
        [MsimEnd, RF_phiEnd] = bSSFPrestorefunc(BSIM, 0, SPGRend);
    else
        MsimEnd = []; RF_phiEnd = [];
    end
...
% For SPGR, catmode = 0;
% I only modified the code when catmode == 0;
if( SPGRcat.catmode==0 )
    plotTEidx = [0:(BSIM.NTR+SPGRend.NendTR-1)].*double(BSIM.NstepTR) + BSIM.TE/BSIM.dt;
elseif( SPGRcat.catmode==1 )
    plotTEidx = SPGRcat.NstepTot+[0:(BSIM.NTR+SPGRend.NendTR-1)]*double(BSIM.NstepTR) + BSIM.TE/BSIM.dt;
    plotTEidx = [1, plotTEidx];
elseif( SPGRcat.catmode==2 )
    % If TR is TR/2 for RAMP_DOWN, this line needs to be changed
    plotTEidx = [0:(SPGRcat.NcatTR+BSIM.NTR+SPGRend.NendTR-1)]*double(BSIM.NstepTR) + BSIM.TE/BSIM.dt;
elseif( SPGRcat.catmode==3 )
    plotTEidx = [0:(SPGRcat.NcatTR+BSIM.NTR+SPGRend.NendTR-1)]*BSIM.NstepTR + BSIM.TE/BSIM.dt;
end
...
if( PLOT_SS )
    ...
    title('SPGR frequency profile: mag');
    xlabel('Hz'); ylabel('Normalized |Mxy|');
    subplot(2,1,2);
    plot( f_vec, angle( MxyTElast ) );
    title('SPGR frequency profile: phase');
    ...
    title('SPGR frequency profile: phased-enc');
    xlabel('Hz'); ylabel('Normalized Mxy');
