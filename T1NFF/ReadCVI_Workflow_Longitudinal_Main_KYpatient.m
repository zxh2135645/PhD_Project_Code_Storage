...
folder_glob = glob(cat(2, base_dir, '/KY_Patient_Data/*'));
...
%for n = starting_point:starting_point
    ...
    %for tp = 7:7
        ...
        xml_glob2 = glob(cat(2, base_dir, '/Updated_XML/',  name, '/', time_point,  '/*.cvi42wsx.xml'));
        if isempty(xml_glob) && isempty(xml_glob2)
            ...
            for con_idx = 1:length(con_cell)
                ...
                %for ll = 1:1
                    ...
                    dicom_glob = glob(cat(2, base_dir, '/KY_Patient_Data/', name, '/', time_point, '/', label, '/*'));
