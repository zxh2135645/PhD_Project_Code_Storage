...
for n = 1:length(Names)
    ...
    for tp = 1:length(time_points)
        ...
        time_point = time_points{end-tp+1};
        tp_dir = cat(2, base_dir, '/ContourData/',  name, '/');
        if ~exist(tp_dir, 'dir')
            disp(cat(2, 'No folder at: ', name, ' ', time_point));
        else
            ...
            for f = 1:length(ff_map)
                str_converted = cat(2, upper(glob_names{f}(1)) , glob_names{f}(2:6) , '-' , glob_names{f}(8:10));
                % f_glob = glob(cat(2,  base_dir, '/FF_Data/',  name, '/', time_point, '/', glob_names{f}, '*.mat'));
                f_glob = glob(cat(2,  base_dir, '/FF_Data/',  name, '/*', str_converted, '*.mat'));
            ...
            if 0
                ...
                for slc = 1:length(roi_in_myo_ff)
                    roi_in_myo_ff_nan = roi_in_myo_ff{slc};
                    remote_in_myo_ff_nan = remote_in_myo_ff{slc};
                    ...
                    metrics(n).TimePoints(tp_count).SliceAnalysis(slc).mean_ff_roi = mean(nonzeros(vec(ff{slc} .* roi_in_myo_ff_nan)), 'omitnan');
                    metrics(n).TimePoints(tp_count).SliceAnalysis(slc).mean_ff_remote = mean(nonzeros(vec(ff{slc} .* remote_in_myo_ff_nan)), 'omitnan');
                    metrics(n).TimePoints(tp_count).SliceAnalysis(slc).std_ff_roi = std(nonzeros(vec(ff{slc} .* roi_in_myo_ff_nan)), 'omitnan');
                    metrics(n).TimePoints(tp_count).SliceAnalysis(slc).std_ff_remote = std(nonzeros(vec(ff{slc} .* remote_in_myo_ff_nan)), 'omitnan');
                    ...
                    fname = cat(2, 'T1entropy_Histo_ROI_', num2str(slc), 'unweighted_original.png');
                    saveas(gcf,cat(2, name_save_dir, '/', fname));
                    ...
                    fname = cat(2, 'T1entropy_Histo_Remote_', num2str(slc), 'unweighted_original.png');
                    saveas(gcf,cat(2, name_save_dir, '/', fname));
                    ...
                    if length(unique(L)) > 2
                        L_cell{count} = cat(2, name, ' Slice ', num2str(slc));
                        count = count + 1;
                    end
                end
