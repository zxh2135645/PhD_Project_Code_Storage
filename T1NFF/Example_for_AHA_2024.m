...
%                 histogram(nonzeros(t1 .* roi_in_myo_t1), 'Normalization', 'Probability');
                ...
                Segn_array = [50 50 50];
                slc_array = [2,3,4];
                E_seg_mat = zeros(sum(Segn_array), 1);
                Groove_array = [45 60 60]-30;
                count = 1;
                for slc = 1:3 
                    Segn = Segn_array(slc);
                    slc_true = slc_array(slc);
                    figure(); imagesc(t1(:,:,slc_true).*myo_t1(:,:,slc_true));
                    Groove = Groove_array(slc);

                    [Segmentpix, stats, Mask_Segn] = AHASegmentation(t1(:,:,slc_true), myo_t1(:,:,slc_true), Segn, Groove);
                    Mask_entropy = zeros(size(Mask_Segn));
                    figure(); 
                    for seg = 1:Segn
                        temp_seg = Segmentpix{seg};
                        bipolar_seg = temp_seg - mean(nonzeros(temp_seg));
                        weighted_map_seg = double(bipolar_seg < 0) .* bipolar_seg + double(bipolar_seg >= 0) .* bipolar_seg;
                        temp_norm_seg = uint8((weighted_map_seg - lb)./ (ub-lb) * 256);

                        non_seg_value = uint8((0 - lb) ./ (ub - lb) * 256);
                        t1_norm_seg_slc = temp_norm_seg;
                        t1_norm_seg_slc(t1_norm_seg_slc == non_seg_value) = [];

                        bins = 20;

                        %figure(); imhist(t1_norm_roi_slc,bins);
                        p_seg = imhist(t1_norm_seg_slc,bins);
                        nonZeros = find(p_seg);
                        len = length((nonZeros));
                        pNonZeros = zeros(1,len);

                        for i = 1:len
                            if p_seg(nonZeros(i)) > 0 % No removal
                                pNonZeros(i) = p_seg(nonZeros(i));
                            end
                        end
                        ...
                        %pNonZeros = pNonZeros ./ sum(p_roi);
                        pNonZeros = nonzeros(pNonZeros ./ sum(pNonZeros));
                        E_seg = -sum(pNonZeros.*log2(pNonZeros));

                        E_seg_mat(count) = E_seg;

                        Mask_entropy(Mask_Segn == seg) = E_seg;

                        count = count + 1;
                    end
                    imagesc(Mask_entropy);
