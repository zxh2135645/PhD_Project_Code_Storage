...
% Truncate to get the last 6 ave
trunc_idx = 6;
one_ave = size(kspace_data,1)/params.NAve;
one_ave_half = one_ave/2;
kspace_data = kspace_data(end-one_ave*trunc_idx+1:end,:,:,:);
pe_indices = pe_indices(end-one_ave*trunc_idx+1:end,:);
nav_data = nav_data(end-one_ave_half*trunc_idx+1:end,:,:,:);
Nread = Nread / params.NAve * trunc_idx;
nav_indices = cutoff_shift:SGblock:Nread;
