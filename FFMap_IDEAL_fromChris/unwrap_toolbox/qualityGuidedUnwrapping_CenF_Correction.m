%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Copyright V¨¦ronique Fortier 2017 - MIT License
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Chris, CenF Correction, 2020/9/7 

function [ im_unwrapped ] = qualityGuidedUnwrapping_CenF_Correction(im_phase, im_mask,qualityCutoff )
...
if (rowref>1 && rowref<matrixSize(1)) && (colref>1 && colref<matrixSize(2)) && (zpoint>1 && zpoint<matrixSize(3))
    if im_mask(rowref-1, colref, zpoint)==1
        adjoin(rowref-1, colref, zpoint)=1; 
    end       
    if im_mask(rowref+1, colref, zpoint)==1 
        adjoin(rowref+1, colref, zpoint)=1; 
    end
    if im_mask(rowref, colref-1, zpoint)==1 
        adjoin(rowref, colref-1, zpoint)=1; 
    end
    if im_mask(rowref, colref+1, zpoint)==1 
        adjoin(rowref, colref+1, zpoint)=1; 
    end
    if im_mask(rowref, colref, zpoint-1)==1 
        adjoin(rowref, colref, zpoint-1)=1; 
    end
    if im_mask(rowref, colref, zpoint+1)==1 
        adjoin(rowref, colref, zpoint+1)=1; 
    end
...
Phase_center = median(im_unwrapped(im_mask~=0));
im_unwrapped = im_unwrapped-2*pi*round(Phase_center/2/pi);
end
