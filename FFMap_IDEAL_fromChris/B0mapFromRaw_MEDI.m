...
%%
addpath(genpath(pwd));
rmpath(genpath(cat(2, pwd, '/Example/')));
rmpath(genpath(cat(2, pwd, '/MEDI_toolbox/')));
FlagMask=1;
FlagSaveB0=1;
FlagLoadFolder=0;
IsManual_Mask=1;
...
for n=1:(length(ls)-2)
    ...
    if length(size(rawdata)) == 6
        ...
        Rawdatapermute=permute(rawdata,[1,3,4,2,5]); % NumRO, NumPE, NumSlices, NumCh, NumAve
    end

    if twix_obj.image.NAve ~= 1
        Rawdatapermute = squeeze(mean(Rawdatapermute, 5));
    end
...
%    end
    ...
    iField=permute(RawdataFT,[1 2 3 5 4])*100; % NumRO, NumPE, NumSlc, NumEco, NumCh
    ...
    [water,fat,iFreq,unwph_uf,unwph,N_std] = ...
        spurs_gc(iField,AllPhasemap(n).TE * 1e-6,f_central,voxel_size,Mask); %iFreq in rad
    ...
    AllPhasemap(n).water = water;
    AllPhasemap(n).fat = fat;
    AllPhasemap(n).iFreq = iFreq;
    AllPhasemap(n).unwph_uf = unwph_uf;
    AllPhasemap(n).unwph = unwph;
    AllPhasemap(n).N_std = N_std;
    AllPhasemap(n).compleximg = [];
    %%
    n
end
