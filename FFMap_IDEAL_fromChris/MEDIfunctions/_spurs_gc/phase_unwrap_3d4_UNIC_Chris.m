function [unwph,iter,erglist] = phase_unwrap_3d4_UNIC_Chris(psi,p,iMag,voxel_size,Mask,varargin)
% Randy Yang 2019 Cedars Modify for correct phase jumping direction
...
%     erg_previous = energy_ho_3d(kappa,psi,base_xy,base_z,p,cliques,disc_bar,disc_bar_z,th,quant);
    ...
    psi_Mask = psi.*Mask;
    erg_previous = energy_ho_3dW4_Chris(voxel_size,iMag,kappa_Mask,psi_Mask,base_xy,base_z,p,cliques,disc_bar,disc_bar_z,th,quant,Mask);
%     erg_previous = 1e10;

    while possible_improvment
        iter = iter + 1;
        fprintf('4-jump move, iter: %d \n', iter);
...
%                 diMag(:,:,t,zz) = ((1./auxiliW(:,:,zz)).^2 + (1./iMag_base(:,:,zz)).^2).^(-0.5);
    ...
    %             B(:,:,t) = clique_energy_ho(abs(-2*pi*jump_size + a(:,:,t)),p,th,quant).*base.*circshift(base,[cliques(t,1),cliques(t,2)]);
                ...
                a(:,:,t,zz) = (4*pi*t_dkappa(:,:,t,zz)-dpsi(:,:,zz)).*base_xy(:,:,zz).*circshift(base_xy(:,:,zz),[cliques(t,1),cliques(t,2)]);
...
%                 
                C(:,:,t,zz) = diMag(:,:,t,zz).*clique_energy_ho(abs(4*pi*jump_size + a(:,:,t,zz))/voxel_size(1),p,th,quant).*base_xy(:,:,zz).*circshift(base_xy(:,:,zz),[cliques(t,1),cliques(t,2)]).*base_disc_bar(:,:,t,zz);
                B(:,:,t,zz) = diMag(:,:,t,zz).*clique_energy_ho(abs(-4*pi*jump_size + a(:,:,t,zz))/voxel_size(1),p,th,quant).*base_xy(:,:,zz).*circshift(base_xy(:,:,zz),[cliques(t,1),cliques(t,2)]).*base_disc_bar(:,:,t,zz);
...
%             diMagz(:,:,zz) = ((1./iMag(:,:,zz-1)).^2 + (1./iMag(:,:,zz)).^2).^(-0.5);
        ...
        for zz = 2:size(psi,3)
            ...
            b(:,:,zz) = (4*pi*dkappa(:,:,zz)-dpsi_z(:,:,zz)).*base_disc_bar_z(:,:,zz);
...
%             BB(:,:,zz) = diMagz(:,:,zz).*clique_energy_ho(abs(-2*pi*jump_size*diMagz(:,:,zz) + b(:,:,zz)),p,th,quant).*base_z(:,:,zz).*base_disc_bar_z(:,:,zz);
        ...
        %kappa_aux(cutside(:,1)) = kappa(cutside(:,1)) + (1 - cutside(:,2))*jump_size;%original
        % RY modify -- can only go positive -- only add 2pi and not
        % negative 2pi RY correct the jump direction
        %R.Y. the majority pixel would stay the same
        ...
        if Majorpix>0.5 % major pixel are 1-> the zero part should jump +
            ...
            kappa_aux(cutside(:,1)) = kappa(cutside(:,1)) - cutside(:,2)*jump_size;
        end

        % CHECK ENERGY IMPROVEMENT
%       erg_actual = energy_ho(kappa_aux,psi,base,p,cliques,disc_bar,th,quant);
        kappa_aux_Mask = kappa_aux.*Mask;
        psi_Mask = psi.*Mask;
        erg_actual = energy_ho_3dW4_Chris(voxel_size,iMag,kappa_aux_Mask,psi_Mask,base_xy,base_z,p,cliques,disc_bar,disc_bar_z,th,quant,Mask);
