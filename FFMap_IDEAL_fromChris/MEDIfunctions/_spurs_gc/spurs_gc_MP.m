...
function [wwater wfat1 wfat2 wfreq wunwph_uf unwphw N_std R2s fitting_error] = spurs_gc_MP(iField,TE,CF,voxel_size,Mask,SUBSAMPLE,dfat,LABEL,ROI)
if nargin<6
    SUBSAMPLE = 1;
end
if nargin<8
...
if LABEL == 1
    ...
    %[wwater wfat wfreq R2s iter model fitting_error] = fit_IDEAL_R2(iField(:,:,:,:), TE, dfat, unwphw./(2*pi*delta_TE),R2s,10);
    [wwater, wfat1, wfat2, wfreq, R2s, iter, model, fitting_error] = fit_MP_IDEAL_R2_Chris(iField(:,:,:,:), TE, [dfat, dfib], unwphw./(2*pi*delta_TE), R2s, 30, ROI);
