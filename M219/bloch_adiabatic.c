...
int blochsim(double *b1real, double *b1imag, 
		...
		double *mx, double *my, double *mz, int mode, double *FrequencyModule, int poscount)
...
// 								df*TWOPI ) * *tsteps;
    ...
    rotz = (*xgrad++ * gammadx + *ygrad++ * gammady + *zgrad++ * gammadz +
				  *FrequencyModule++ + df*TWOPI ) * *tsteps;
    
	rotx = (- *b1real++ * GAMMA * *tsteps);
	roty = (+ *b1imag++ * GAMMA * *tsteps++);
	calcrotmat(rotx, roty, rotz, rotmat);
    
//     if (poscount == 1 & tcount%100 == 1)
//     {
//     printf("rotmat = [%6.6f  %6.6f  %6.6f %6.6f %d %6.6f] \n",rotx, roty, rotz,*FrequencyModule, tcount, *tsteps);
//     }
//     
//     if (poscount == 2000 & tcount%100 == 1)
//     {
//     printf("mcurr0 = [%6.3f  %6.3f  %6.3f %d] \n",mcurr0[0],mcurr0[1],mcurr0[2],tcount);
//     }
...
//     mcurr0[0] = mcurr1[0];		/* Set starting x magnetization */
//     mcurr0[1] = mcurr1[1];		/* Set starting y magnetization */
//     mcurr0[2] = mcurr1[2];		/* Set starting z magnetization */
//     
...
//     if (poscount == 1 & tcount%100 == 1)
//     {
//     printf("mcurr1 = [%6.3f  %6.3f  %6.3f %d ] \n",mcurr1[0],mcurr1[1],mcurr1[2],tcount);
//     }
		/* 	Decay	*/
...
//     if (poscount == 1 & tcount%100 == 1)
//     {
//     printf("mcurr0_afterdecay = [%6.3f  %6.3f  %6.3f %d] \n",mcurr0[0],mcurr0[1],mcurr0[2],tcount);
//     }
//     if (poscount == 1 )
//     {
//     printf("e1 e2 = [%6.3f  %6.3f %d ] \n",*e1, *e2, tcount);
//     }
...
//     
//     
//     t1vec1[2] = t1vec[2] * *tsteps++;
//     addvecs(mcurr1, t1vec1, mcurr0);
//     
// 	
...
int blochsimfz(double *b1real, double *b1imag, double *xgrad, double *ygrad, double *zgrad, 
		...
		double *mx, double *my, double *mz, int mode, double *fmodule)
...
for (fcount=0; fcount < nfreq; fcount++)
	...
	if (mode == 3)	/* Steady state AND record all time points. */
		...
		blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
			...
			*dzptr, mx, my, mz, 1,fmodule,poscount);

		blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
			e1, e2, *dfreq, *dxptr++, *dyptr++, 
			*dzptr++, mx, my, mz, 2,fmodule,poscount);
		}
	else
		{
		blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
			e1, e2, *dfreq, *dxptr++, *dyptr++, 
			*dzptr++, mx, my, mz, mode, fmodule,poscount);
		}
...
double *freqmodule; /*frequency modulation*/
...
freqmodule = mxGetPr(prhs[8]);

// printf("%e, frquencymodule.\n", *(freqmodule+100));
...
blochsimfz(b1r,b1i,gx,gy,gz,tp,ntime,t1,t2,df,nf,dx,dy,dz,npos,mx,my,mz,md,freqmodule);
