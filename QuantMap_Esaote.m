...
dicom_dir = uigetdir;
dicom_glob = glob(cat(2, dicom_dir, '/*_SE_TE*'));
dicom_glob_reorder = dicom_glob([1,2,3]); % JESSE
...
figure();
imagesc(t2_map(:,:,10)); axis image; caxis([0 100]); axis off;
colormap(brewermap([],'RdYlBu')); colorbar;

%% T1-IR-SE
dicom_glob = glob(cat(2, dicom_dir, '/*_IRSE_TI*'));
dicom_glob_reorder = dicom_glob([1,2,3]); % JESSE, GINGER, PAPRIKA
...
figure();
imagesc(t1_map(:,:,10)); axis image; caxis([20 200]); axis off;
